...
const declareIfNot_NotWait = async (
    ...
    try {
        // Add higher tip for Sepolia to avoid mempool eviction due to network congestion
        const declareOptions =
            networkName === "devnet"
                ? { ...options, tip: 1000n }
                : networkName === "sepolia"
                ? { ...options, tip: 100000n } // Much higher tip for Sepolia (100x devnet)
                : { ...options };
        ...
        if (networkName === "sepolia" || networkName === "mainnet") {
            ...
            if (receiptAny.execution_status !== "SUCCEEDED") {
                const revertReason =
                    receiptAny.revert_reason || "Unknown reason";
                throw new Error(
                    red(
                        `Declaration failed or reverted. Reason: ${revertReason}`
                    )
                );
            }
...
const deployContract = async (
    ...
    try {
        compiledContractCasm = JSON.parse(
            fs
                .readFileSync(
                    findContractFile(contract, "compiled_contract_class")
                )
                .toString("ascii")
        );
    } catch (error) {
        if (error.message.includes("Could not find")) {
            console.error(
                red(
                    `The contract "${contract}" doesn't exist or is not compiled`
                )
            );
        } else {
            console.error(
...
const executeDeployCalls = async (options?: UniversalDetails) => {
    ...
    try {
        ...
        console.error(
            red("Deployment tx execution failed: error details below")
        );
        console.error(e);
